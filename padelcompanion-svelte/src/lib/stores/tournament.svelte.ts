...
import {
  generateFinals,
  generateSemifinals,
} from "$lib/tournament/scoring/playoffGenerator";
import {
  recordCourtResult,
  advanceRound,
  clearSide,
...
import { updateMatchResult } from "$lib/tournament/bracket/bracketCore";
...
class TournamentStore {
  ...
  load() {
    const saved = StorageService.getItem(STORAGE_KEY) as any;
    if (saved) {
      const merged = {
        ...DEFAULT_STATE,
        ...saved,
  ...
  save() {
    if (this.isLoaded) {
      StorageService.setItem(STORAGE_KEY, this.state);
  ...
  addLatePlayer(player: Player) {
    ...
    const defaultDivisionId =
      this.state.format === "division"
        ? this.state.players[0]?.divisionId || this.state.players[0]?.division || (this.state as any).divisions?.[0]?.id || "A"
        : undefined;
    const defaultDivisionName =
      this.state.format === "division"
        ? this.state.players[0]?.division || (this.state as any).divisions?.find?.((div: any) => div.id === defaultDivisionId)?.name || "A"
        : undefined;
  ...
  advanceWinnersCourtRound(side: string) {
    if (!this.state.winnersCourt) return;
  ...
  completeRound() {
    ...
    if (
      ...
      if (this.state.allRounds && nextRoundIndex < this.state.allRounds.length) {
        ...
        nextRoundBase = generateFinals(newSchedule[currentRoundIdx] as any, this.state.players as any) as any;
      }
    }
