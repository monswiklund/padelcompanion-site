...
interface StoredTournamentSession extends TournamentSessionResponse {}
...
function readLocalSessions(): Record<string, StoredTournamentSession> {
  return (
    StorageService.getItem<Record<string, StoredTournamentSession>>(
      LOCAL_SESSIONS_KEY,
      {},
    ) || {}
  );
...
function writeLocalSessions(
  sessions: Record<string, StoredTournamentSession>,
): void {
  StorageService.setItem(LOCAL_SESSIONS_KEY, sessions);
}

function findSessionByShareCode(
  shareCode: string,
): StoredTournamentSession | undefined {
  ...
  try {
    ...
    if (
      ...
      (error instanceof Error &&
        /Failed to fetch|NetworkError|404|405|500|502|503|504/.test(
          error.message,
        ))
    ) {
      return fallback();
