 ...
 */
export function generateWinnersCourt(players: WCPlayer[], config: WCConfig): WinnersCourtState {
 ...
 */
export function allCourtsComplete(state: WinnersCourtState, side: string): boolean {
...
export function advanceRound(
	...
	const allPlayers = [...processingWinners, ...sideState.queue, ...processingLosers];
	const courtCount = sideState.courts.length;
	const { courts: newCourts, queue: newQueue } = createCourts(allPlayers, courtCount);
 ...
 */
export function clearSide(state: WinnersCourtState, side: string): WinnersCourtState | null {
	if (!state.sides[side]) return state;
