...
function getPlannedStartTimestamp(plannedStartTime?: string): number {
	const [hours, minutes] = (plannedStartTime || DEFAULT_START_TIME).split(':').map(Number);
	const date = new Date();
...
function getEstimatedRoundDurationSeconds(state: TournamentState, roundIdx: number): number {
	const baseDurationMinutes = state.matchDuration || DEFAULT_MATCH_DURATION_MINUTES;
	const shouldIncludeWarmupBuffer =
		state.format === 'division' && roundIdx < EARLY_ROUND_BUFFER_ROUNDS;

	return (baseDurationMinutes + (shouldIncludeWarmupBuffer ? EARLY_ROUND_BUFFER_MINUTES : 0)) * 60;
}

export function getEstimatedRoundStartDate(state: TournamentState, roundIdx: number): Date {
	...
	for (let idx = 0; idx < roundIdx; idx++) {
		...
		if (round?.completed) {
			cursor += (round.durationSeconds || getEstimatedRoundDurationSeconds(state, idx)) * 1000;
			continue;
		}

		if (isActiveRound) {
			cursor += Math.max(getEstimatedRoundDurationSeconds(state, idx), activeElapsedSeconds) * 1000;
			continue;
		}
...
export function formatEstimatedRoundStart(state: TournamentState, roundIdx: number): string {
