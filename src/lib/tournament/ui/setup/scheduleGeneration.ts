...
import { showConfirmModal, showAlertModal, showCountdown } from '../../core/modals';
...
export async function generateSchedule(): Promise<any> {
	...
	if (state.players.length < minPlayers) {
		showToast(`Not enough ${isTeam ? 'teams' : 'players'} (min ${minPlayers})`, 'error');
		return null;
	}

	const playersNeededPerCourt =
		state.format === 'team' || state.format === 'teamMexicano' || state.format === 'division'
			? 2
			: 4;
	...
	const maxPossibleCourts = Math.floor(state.players.length / playersNeededPerCourt);
	...
	if (state.format === 'americano') {
		...
		const teams = state.players.map((p) => ({ id: p.id, name: p.name }));
		const { generateBracket } = await import('../../bracket/bracketGeneration');
		tournament.updateField('bracket', generateBracket(teams));
		generatedSchedule = [];
	} else if (state.format === 'winners-court') {
		const { generateWinnersCourt } = await import('../../winnersCourt/winnersCourtCore');
		const wcPlayers = state.players.map((p) => ({ id: p.id, name: p.name, side: null }));
		const winnersCourt = generateWinnersCourt(wcPlayers, {
			poolCount: 1,
			courtsPerPool: { A: Math.floor(state.players.length / 4) },
			twist: true
		});
		tournament.updateField('winnersCourt', winnersCourt);
		generatedSchedule = [];
	} else if (state.format === 'teamMexicano') {
		generatedSchedule = generateTeamMexicanoFirstRound(state.players, effectiveCourts) as any[];
 ...
 */
export function endTournament(showFinalStandingsCallback?: (standings: any[]) => void): void {
	showConfirmModal(
		'End Tournament?',
