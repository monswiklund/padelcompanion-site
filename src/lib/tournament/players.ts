...
import { state, saveState, type Player, type PreferredPartner } from './core/state';
...
export function addPlayer(name: string): boolean {
	...
	if (state.players.some((p: Player) => p.name.toLowerCase() === trimmedName.toLowerCase())) {
...
export function importPlayers(text: string): ImportResult {
	...
	if (rawNames.length === 0) return { added: 0, duplicates: 0, hitLimit: false };
	...
	for (const name of rawNames) {
		...
		if (state.players.some((p: Player) => p.name.toLowerCase() === name.toLowerCase())) {
			duplicates++;
			continue;
		}
...
export function getAvailablePlayersForPairing(): Player[] {
	...
	state.preferredPartners.forEach((pair: PreferredPartner) => {
...
export function addPreferredPair(): void {
	...
	state.preferredPartners.push({
		id: createId(),
		player1Id: available[0].id as string,
		player2Id: available[1].id as string
	});
...
export function removePreferredPair(pairId: string | number): void {
	state.preferredPartners = state.preferredPartners.filter(
		(p: PreferredPartner) => p.id !== pairId
	);
	saveState();
}
...
export function updatePreferredPair(
	...
	const pair = state.preferredPartners.find((p: PreferredPartner) => p.id === pairId);
	if (pair) {
		if (which === 1) pair.player1Id = playerId as string;
		else pair.player2Id = playerId as string;
		saveState();
	}
}
