	...
	return Object.fromEntries(
		sorted.map((div) => [
			...
			Array.from({ length: Math.max(1, div.courts || 1) }, (_, index) => {
				const value = divisionCourtNames[div.id]?.[index] ?? divisionCourtNames[div.name]?.[index];
...
export function getCourtDisplayName(
	...
	if (state.format === 'division' && divisionIdOrName) {
		const localCourtNumber = getLocalDivisionCourtNumber(state as any, courtNum, divisionIdOrName);
