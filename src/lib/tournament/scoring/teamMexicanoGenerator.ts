 ...
 */
export function generateTeamMexicanoFirstRound(players: Player[], courts: number): Round[] {
 ...
 */
function getRepeatCount(team1: Player, team2: Player, leaderboard: Player[]): number {
 ...
 */
export function generateTeamMexicanoNextRound(config: TeamMexicanoNextRoundConfig): Round {
	const {
		leaderboard,
		manualByes,
		courts,
	...
	const pairs = applyPairingStrategy(available, pairingStrategy, maxRepeats, leaderboard);
	...
	const byes = [...manualByePlayers, ...available.filter((t) => !playersInMatches.has(t.id))];
