import type { Match, Player, Round, TournamentState } from '../core/state';
...
export function generateSemifinals(state: TournamentState): Round {
	...
	divisions.forEach((div) => {
		...
		console.log(
			`[Playoff] Division ${div} standings:`,
			sorted.map((p) => `${p.name} (${p.matchPoints}pts)`)
		);
 ...
 */
export function generateFinals(semifinalRound: Round): Round {
	const matches: Match[] = [];
	let courtCounter = 1;
