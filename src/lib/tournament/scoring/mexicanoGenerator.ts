...
function getPreferredPartnerId(
	pid: string | number,
	preferredPartners: PreferredPartner[] = []
): string | number | null {
	if (!preferredPartners) return null;
	const pair = preferredPartners.find(
...
function buildEntities(
	...
	players.forEach((p) => {
		...
		const partnerId = getPreferredPartnerId(p.id, preferredPartners);
...
export function generateMexicanoFirstRound(
	...
	const availableEntities = buildEntities(players, new Set(), preferredPartners);
...
export interface MexicanoNextRoundConfig {
	leaderboard: Player[];
...
export function generateMexicanoNextRound(config: MexicanoNextRoundConfig): Round {
	const {
		leaderboard,
		manualByes,
		courts,
	...
	const playersNeeded = courts * 4;
	const manualByIds = new Set<string | number>(manualByes);
	const allPlayers = [...leaderboard];

	const availableEntities = buildEntities(allPlayers, manualByIds, preferredPartners);
	...
	pairSinglesWithStrategy(singles, teams, leaderboard, pairingStrategy, maxRepeats, strictStrategy);

	const remainingSingles = singles.filter((s) => !teams.some((t) => t.includes(s)));
	...
	const { matches, pInMatch } = assignCourts(teamsWithLocks, normalTeams, courts);
...
function pairSinglesWithStrategy(
	...
	for (; i < singles.length - 3; i += 4) {
		...
		const selectedPairing = selectPairing(
			pairings,
			leaderboard,
			pairingStrategy,
			maxRepeats,
...
function selectPairing(
	...
	const patternHard = pairingStrategy !== 'optimal' && strictStrategy;
	const maxRep = maxRepeats;
	...
	const wasLastPartner = (pid1: string | number, pid2: string | number) => {
		const player = leaderboard.find((p) => p.id === pid1);
		return player?.playedWith?.at(-1) === pid2;
	};
	...
	const scoredPairings = pairings.map((pairing) => {
		...
		const immediateRepeatPenalty =
			Number(wasLastPartner(pairing.team1[0].id, pairing.team1[1].id)) +
			Number(wasLastPartner(pairing.team2[0].id, pairing.team2[1].id));

		const team1Rank = (pairing.team1[0].points || 0) + (pairing.team1[1].points || 0);
		const team2Rank = (pairing.team2[0].points || 0) + (pairing.team2[1].points || 0);
		const rankingImbalance = Math.abs(team1Rank - team2Rank);

		const violatesRepeats = maxRep < 99 && repeatPenalty > maxRep;
		const isPreferred = pairing.name === pairingStrategy;
	...
	if (pairingStrategy === 'optimal') {
		const sorted = [...scoredPairings].sort((a, b) => {
			if (a.immediateRepeatPenalty !== b.immediateRepeatPenalty) {
				return a.immediateRepeatPenalty - b.immediateRepeatPenalty;
			}
			if (a.repeatPenalty !== b.repeatPenalty) return a.repeatPenalty - b.repeatPenalty;
			if (a.rankingImbalance !== b.rankingImbalance) return a.rankingImbalance - b.rankingImbalance;
			return a.tieBreaker - b.tieBreaker;
		});
		return sorted[0];
	}

	const userChoice = scoredPairings.find((p) => p.isPreferred) || scoredPairings[0];

	if (!userChoice.violatesRepeats && userChoice.immediateRepeatPenalty === 0) {
		return userChoice;
	}
	...
	const validOptions = scoredPairings.filter(
		(p) => !p.violatesRepeats && p.immediateRepeatPenalty === 0
	);
