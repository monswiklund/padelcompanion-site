...
import { getTeamById, type BracketMatch, type BracketTeam } from './bracketGeneration';
...
export function updateMatchResult(
	...
	const match = (state.bracket.matches as BracketMatch[]).find((m) => m.id === matchId);
	if (!match) return;
	...
	if (score1 !== null && score2 !== null && score1 !== score2) {
		...
		if (match.nextMatchId) {
			...
			if (nextMatch && match.winnerId) {
				const winner = getTeamById(state.bracket.teams as BracketTeam[], match.winnerId);
				if (winner) {
...
export function isBracketComplete(): boolean {
	...
	const numRounds = (state.bracket as any).numRounds || 0;
	const finalMatch = (state.bracket.matches as BracketMatch[]).find((m) => m.round === numRounds);
