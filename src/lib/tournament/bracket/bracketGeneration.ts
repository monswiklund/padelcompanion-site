...
export function updateBracketResult(
	bracket: Bracket,
	matchId: number,
	score1: number,
	score2: number
	...
	match.score1 = score1;
	match.score2 = score2;
	match.completed = score1 !== score2;
	match.winnerId = score1 > score2 ? match.team1Id : score2 > score1 ? match.team2Id : null;

	clearDownstream(matches, match.id);

	if (match.winnerId) {
		advanceWinner(matches, match, bracket.teams);
	}
 ...
 */
export function seedTeams(teams: BracketTeam[], bracketSize: number): (BracketTeam | null)[] {
	...
	const noSide = teams.filter((t) => !t.side || (t.side !== 'A' && t.side !== 'B'));
 ...
 */
export function getTeamById(teams: BracketTeam[], id: string): BracketTeam | undefined {
	return teams.find((t) => t.id === id);
}

function advanceWinner(matches: BracketMatch[], match: BracketMatch, teams: BracketTeam[]): void {
...
function clearDownstream(matches: BracketMatch[], matchId: number): void {
	const match = matches.find((m) => m.id === matchId);
