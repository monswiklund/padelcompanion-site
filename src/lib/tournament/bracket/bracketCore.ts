 ...
 */
function seedTeamsInOrder(teams: BracketTeam[], bracketSize: number): (BracketTeam | null)[] {
 ...
 */
function isEmptySource(match: BracketMatch | undefined): boolean {
	return !!match && !match.team1 && !match.team2 && !match.winner;
}

function autoAdvanceMatch(
	match: BracketMatch,
	prevMatch1?: BracketMatch,
	prevMatch2?: BracketMatch
): BracketMatch {
	const team1BlockedByPendingSource = !match.team1 && !!prevMatch1 && !isEmptySource(prevMatch1);
	const team2BlockedByPendingSource = !match.team2 && !!prevMatch2 && !isEmptySource(prevMatch2);

	if (match.team1 && !match.team2 && !team2BlockedByPendingSource) {
		return { ...match, winner: match.team1 };
	} else if (!match.team1 && match.team2 && !team1BlockedByPendingSource) {
...
function generateMatchesForBracket(
	...
	for (let round = 2; round <= numRounds; round++) {
		...
		for (let i = 0; i < matchCount; i++) {
			...
			match = autoAdvanceMatch(match, prevMatch1, prevMatch2);
...
export function generateMultiPoolBracket(
	...
	for (let i = 0; i < poolCount; i++) {
		...
		if (poolTeams.length > 0) {
			...
			const matches = generateMatchesForBracket(seeded, numRounds, currentIdOffset);
...
export function applyAssignment(
	...
	if (strategy === 'random') {
		...
		return teams.map((t) => {
			...
			const poolIndex = Math.min(Math.floor(shuffledIndex / teamsPerPool), poolCount - 1);
...
export function generateDualBracket(
	...
	const matchesA = teamsA.length > 1 ? generateMatchesForBracket(seededA, numRoundsA, 0) : [];
	const maxIdA = matchesA.length > 0 ? Math.max(...matchesA.map((m) => m.id)) : 0;
	const matchesB = teamsB.length > 1 ? generateMatchesForBracket(seededB, numRoundsB, maxIdA) : [];
	const maxIdB = matchesB.length > 0 ? Math.max(...matchesB.map((m) => m.id)) : maxIdA;
	...
	if (sharedFinal) {
		grandFinal = {
			...
			winner: null,
			nextMatchId: null,
			prevMatch1Id: matchesA.length > 0 ? matchesA[matchesA.length - 1].id : null,
			prevMatch2Id: matchesB.length > 0 ? matchesB[matchesB.length - 1].id : null
		};
...
export function updateMatchResult(
	...
	const propagateWinner = (matches: BracketMatch[], updatedMatchId: number): BracketMatch[] => {
		const updatedMatch = matches.find((m) => m.id === updatedMatchId);
		if (!updatedMatch || !updatedMatch.winner || !updatedMatch.nextMatchId) return matches;
	...
	if ((bracket as any).isMultiPool) {
		...
		if (grandFinal) {
			...
			if (matchId === grandFinal.id) {
				const winner =
					score1 > score2 ? grandFinal.team1 : score2 > score1 ? grandFinal.team2 : null;
	...
	if ((bracket as any).isDoubleElimination) {
		...
		if (matchId === grandFinal.id) {
			const winner = score1 > score2 ? grandFinal.team1 : score2 > score1 ? grandFinal.team2 : null;
			grandFinal = { ...grandFinal, score1, score2, winner };
		}
	...
	if ((bracket as any).isDualBracket) {
 ...
 */
export function generateDoubleEliminationBracket(teams: BracketTeam[]): DoubleEliminationBracket {
	if (teams.length < 2) {
		throw new Error('Need at least 2 teams for a bracket');
	}
	...
	const losersFinal = losersMatches.length > 0 ? losersMatches[losersMatches.length - 1] : null;

	const grandFinalId = currentIdOffset + 1;
