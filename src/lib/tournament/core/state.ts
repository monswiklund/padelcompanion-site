...
export function loadState(): boolean {
	...
	try {
		...
		state.players = Array.isArray(data.players) ? data.players.slice(0, 200) : [];
		...
		state.pointsPerMatch = Math.max(1, Math.min(999, data.pointsPerMatch || 24));
		...
		state.hideLeaderboard = data.hideLeaderboard !== undefined ? data.hideLeaderboard : true;
