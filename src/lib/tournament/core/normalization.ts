...
import type { TournamentState, DivisionConfig } from './state';
...
export function normalizeTournamentState(state: TournamentState): TournamentState {
	...
	let nextOrder =
		state.divisions.length > 0 ? Math.max(...state.divisions.map((d) => d.order ?? 0)) + 1 : 0;
	...
	state.divisionCourtNames = Object.fromEntries(
		state.divisions.map((div) => [
			...
			Array.from({ length: Math.max(1, div.courts || 1) }, (_, index) => {
				const value = existingCourtNames[div.id]?.[index] ?? existingCourtNames[div.name]?.[index];
