<script lang="ts">
	...
	async function handleGenerate() {
		tournament.updateField('winnersCourt', { sides: {}, twist });
		await generateSchedule();
	}

	onMount(() => {
		tournament.initForFormat('winners-court');
		window.scrollTo({ top: 0, behavior: 'instant' });
	});
</script>

<svelte:head>
	<title>Winners Court Tournament | Padel Companion</title>
	<meta
		name="description"
		content="Create Winners Court padel tournaments with court movement, live scoring, and standings."
	/>
	<link rel="canonical" href="https://padelcompanion.se/tournament/winners-court/" />
	<meta property="og:type" content="website" />
	<meta property="og:url" content="https://padelcompanion.se/tournament/winners-court/" />
	<meta property="og:title" content="Winners Court Tournament | Padel Companion" />
	<meta property="og:description" content="Create Winners Court padel tournaments." />
			...
			<div transition:fade={{ duration: 250 }} class="mx-auto max-w-2xl space-y-4 pt-6">
				...
				<div class="mb-6 text-center">
					...
					<h1 class="font-display text-3xl font-black tracking-tight text-white uppercase">
						{tsState.tournamentName || 'WINNERS COURT'}
					</h1>
							...
							<div in:fade={{ duration: 180 }} class="space-y-6 text-left">
								...
								<div class="flex flex-col gap-2">
									...
									<input
										type="text"
										id="t-name"
										value={tsState.tournamentName || 'Winners Court'}
										oninput={(e) =>
								...
								<div class="grid grid-cols-1 gap-4 sm:grid-cols-2">
									...
									<div class="flex flex-col gap-2">
										...
										>
										<div
											class="flex items-center justify-between rounded-xl border border-white/10 bg-black/30 px-4 py-2.5"
										>
											<span class="text-xs font-bold text-[#f5f7f8]">Split Partners</span>
											<Switch.Root
												checked={twist}
												onCheckedChange={(v) => (twist = v)}
												class="relative h-[44px] w-[44px] cursor-pointer rounded-full border border-white/15 transition-all data-[state=checked]:bg-[#007AFF] data-[state=unchecked]:bg-white/5"
											>
												<Switch.Thumb
													class="absolute top-1/2 left-1 h-5 w-5 -translate-y-1/2 rounded-full bg-white transition-transform duration-200 data-[state=checked]:translate-x-[14px] data-[state=checked]:bg-black"
												/>
											</Switch.Root>
										</div>
									</div>

									<!-- Active Courts (Calculated display) -->
									<div class="flex flex-col gap-2">
										<span
											class="text-[10px] font-black tracking-wider text-muted-foreground uppercase"
											>Active Courts</span
										...
										>
											<span class="text-xs font-bold text-[#f5f7f8]">Courts Needed</span>
											<span class="text-xs font-black text-[#007AFF]"
												>{Math.floor(playerCount / 4)} Courts</span
											>
										</div>
									</div>
								</div>

								<!-- Number of Players Slider (multiple of 4 for Winners Court) -->
										...
										<div class="flex items-center gap-1.5">
											<input
												...
												oninput={(e) => {
													...
													const rounded = Math.min(128, Math.max(4, Math.round(val / 4) * 4));
								...
								>
									...
									<p class="text-xs leading-relaxed font-bold tracking-wider uppercase">
										Winners Court is a dynamic format where the winners of each match move to a
										"higher" court, and losers move to a "lower" court.
									</p>
								</div>
							</div>
