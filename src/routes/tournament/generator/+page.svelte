<script lang="ts">
	import TournamentNav from '$lib/components/tournament/TournamentNav.svelte';
	import TournamentIdentity from '$lib/components/tournament/TournamentIdentity.svelte';
	import HistorySection from '$lib/components/tournament/HistorySection.svelte';
	import TournamentConfig from '$lib/components/tournament/TournamentConfig.svelte';
	import PlayerList from '$lib/components/tournament/PlayerList.svelte';
	import PreferredPartners from '$lib/components/tournament/PreferredPartners.svelte';
	import TournamentActiveView from '$lib/components/tournament/TournamentActiveView.svelte';
	import GlassCard from '$lib/components/ui/GlassCard.svelte';
	import Button from '$lib/components/ui/Button.svelte';
	import Icons from '$lib/components/Icons.svelte';

	import { tournament } from '$lib/stores/tournament.svelte';
	import { generateSchedule } from '$lib/tournament/ui/setup/scheduleGeneration';
	import { createId } from '$lib/shared/utils';
	import { fade } from 'svelte/transition';
	import { onMount } from 'svelte';

	const tsState = $derived(tournament.state);
	const isLocked = $derived(tsState.isLocked);
	const playerCount = $derived(tsState.players.length);
	const isTeam = $derived(tsState.format === 'team' || tsState.format === 'teamMexicano');
	const minPlayers = $derived(isTeam ? 2 : 4);
	const isReadyToGenerate = $derived(playerCount >= minPlayers);
	const displayedTournamentName = $derived(
		isAutoSetupName(tsState.tournamentName) || !tsState.tournamentName
			? getSetupTitle(tsState.format)
			: tsState.tournamentName
	);

	let activeStep = $state<1 | 2 | 3>(1);

	$effect(() => {
		if (isLocked) {
			activeStep = 3;
		} else if (activeStep === 3) {
			activeStep = 1;
		}
	});

	function handleConfigChange(key: any, value: any) {
		if (key === 'format') {
			const shouldSyncName = !tsState.tournamentName || isAutoSetupName(tsState.tournamentName);
			tournament.updateField(key, value);
			if (shouldSyncName) {
				tournament.updateField('tournamentName', getSetupTitle(value));
			}
			return;
		}
		tournament.updateField(key, value);
	}

	function handleSliderChange(val: number) {
		const currentCount = tsState.players.length;
		if (val > currentCount) {
			const diff = val - currentCount;
			for (let i = 0; i < diff; i++) {
				const nextNum = currentCount + i + 1;
				tournament.addPlayer({
					id: createId(),
					name: isTeam ? `Team ${nextNum}` : `Player ${nextNum}`,
					lockedCourt: null
				});
			}
		} else if (val < currentCount) {
			const diff = currentCount - val;
			for (let i = 0; i < diff; i++) {
				const lastPlayer = tsState.players[tsState.players.length - 1];
				if (lastPlayer) {
					tournament.removePlayer(lastPlayer.id);
				}
			}
		}
	}

	function getSetupTitle(format: string): string {
		switch (format) {
			case 'mexicano':
				return 'Mexicano Setup';
			case 'team':
				return 'Team Americano Setup';
			case 'teamMexicano':
				return 'Team Mexicano Setup';
			case 'americano':
			default:
				return 'Americano Setup';
		}
	}

	function isAutoSetupName(name: string): boolean {
		return [
			'americano setup',
			'mexicano setup',
			'team americano setup',
			'team mexicano setup',
			'team setup'
		].includes(name.trim().toLowerCase());
	}

	// PlayerList callbacks
	function handleAddPlayer(name: string) {
		tournament.addPlayer({ id: createId(), name, lockedCourt: null });
	}

	// Remove player by relative index
	function handleRemovePlayer(index: number) {
		const player = tsState.players[index];
		if (player) tournament.removePlayer(player.id);
	}

	function handleClearPlayers() {
		tournament.clearPlayers();
	}

	function handleImportPlayers(text: string) {
		const names = text
			.split('\n')
			.map((n) => n.trim())
			.filter(Boolean);
		const existingNames = new Set(tsState.players.map((p) => p.name.toLowerCase()));
		names.forEach((name) => {
			if (!existingNames.has(name.toLowerCase())) {
				tournament.addPlayer({ id: createId(), name, lockedCourt: null });
				existingNames.add(name.toLowerCase());
			}
		});
	}

	function handleReorderPlayers(from: number, to: number) {
		const players = [...tsState.players];
		const [moved] = players.splice(from, 1);
		players.splice(to, 0, moved);
		tournament.updateField('players', players);
	}

	// PreferredPartners callbacks
	function handleAddPair(p1Id: string | number, p2Id: string | number) {
		const pairs = [
			...tsState.preferredPartners,
			{
				id: createId(),
				player1Id: String(p1Id),
				player2Id: String(p2Id)
			}
		];
		tournament.updateField('preferredPartners', pairs);
	}

	function handleRemovePair(pairId: string) {
		const pairs = tsState.preferredPartners.filter((p) => p.id !== pairId);
		tournament.updateField('preferredPartners', pairs);
	}

	async function handleGenerate() {
		await generateSchedule();
	}

	onMount(() => {
		const saved = localStorage.getItem('tournament-state');
		let activeFormat: any = 'americano';
		if (saved) {
			try {
				const parsed = JSON.parse(saved);
				if (['americano', 'mexicano', 'team', 'teamMexicano'].includes(parsed.format)) {
					activeFormat = parsed.format;
				}
			} catch {
				activeFormat = 'americano';
			}
		}
		tournament.initForFormat(activeFormat);
		if (!tournament.state.tournamentName || isAutoSetupName(tournament.state.tournamentName)) {
			tournament.updateField('tournamentName', getSetupTitle(tournament.state.format));
		}
		window.scrollTo({ top: 0, behavior: 'instant' });
	});
</script>

<svelte:head>
	<title>Tournament Generator | Padel Companion</title>
	<meta
		name="description"
		content="Generate Americano and Mexicano padel tournament schedules with courts, scoring, timing, and live standings."
	/>
	<link rel="canonical" href="https://padelcompanion.se/tournament/generator/" />
	<meta property="og:type" content="website" />
	<meta property="og:url" content="https://padelcompanion.se/tournament/generator/" />
	<meta property="og:title" content="Tournament Generator | Padel Companion" />
	<meta
		property="og:description"
		content="Generate padel tournament schedules and run live standings in your browser."
	/>
	<meta property="og:image" content="https://padelcompanion.se/assets/app-icon.jpeg" />
</svelte:head>

<div class="noise relative min-h-dvh overflow-x-clip bg-background pb-12 font-sans text-foreground">
	<TournamentNav />

	<div class="relative z-10 container mx-auto max-w-[1280px] px-3 pt-2 sm:px-4 2xl:max-w-[1380px]">
		{#if isLocked}
			<!-- Running active tournament state -->
			<div transition:fade={{ duration: 180 }}>
				<TournamentActiveView />
			</div>
		{:else}
			<!-- Setup Mode: beautiful unified tabbed card matching mockup design -->
			<div transition:fade={{ duration: 250 }} class="mx-auto max-w-2xl space-y-4 pt-6">
				<!-- Header section / Title -->
				<div class="mb-6 text-center">
					<span
						class="mb-2 inline-flex items-center gap-1.5 rounded-full border border-[#007AFF]/20 bg-[#007AFF]/5 px-3 py-1 text-[10px] font-black tracking-wider text-[#007AFF] uppercase"
					>
						<span class="h-1.5 w-1.5 animate-pulse rounded-full bg-[#007AFF]"></span>
						Tournament Setup
					</span>
					<h1 class="font-display text-3xl font-black tracking-tight text-white uppercase">
						{displayedTournamentName}
					</h1>
					{#if tsState.tournamentNotes}
						<p class="mx-auto mt-1 max-w-md text-xs text-muted-foreground">
							{tsState.tournamentNotes}
						</p>
					{/if}
				</div>

				<GlassCard
					variant="premium"
					padding="none"
					class="relative w-full overflow-hidden border border-white/10 shadow-2xl"
				>
					<!-- Background accent glow -->
					<div
						class="pointer-events-none absolute -top-20 -right-20 -z-10 h-64 w-64 rounded-full bg-[#007AFF]/5 blur-[80px]"
					></div>

					<!-- Tabs Header -->
					<div class="flex border-b border-white/5 bg-white/[0.01]">
						<button
							type="button"
							onclick={() => (activeStep = 1)}
							class="flex-1 cursor-pointer border-b-2 py-4 text-center text-xs font-black tracking-wider uppercase transition-all select-none
								{activeStep === 1
								? 'border-[#007AFF] bg-white/[0.02] text-[#007AFF]'
								: 'border-transparent text-[#a7b0b8] hover:text-[#f5f7f8]'}"
						>
							<span class="mr-1.5 opacity-50">1.</span>
							Settings
						</button>
						<button
							type="button"
							onclick={() => (activeStep = 2)}
							class="flex-1 cursor-pointer border-b-2 py-4 text-center text-xs font-black tracking-wider uppercase transition-all select-none
								{activeStep === 2
								? 'border-[#007AFF] bg-white/[0.02] text-[#007AFF]'
								: 'border-transparent text-[#a7b0b8] hover:text-[#f5f7f8]'}"
						>
							<span class="mr-1.5 opacity-50">2.</span>
							Players
						</button>
						<button
							type="button"
							disabled
							class="flex-1 cursor-not-allowed border-b-2 border-transparent py-4 text-center text-xs font-black tracking-wider text-[#a7b0b8]/30 uppercase transition-all select-none"
						>
							<span class="mr-1.5 opacity-30">3.</span>
							Match Schedule
						</button>
					</div>

					<!-- Tab Content area -->
					<div class="flex min-h-[380px] flex-col justify-between gap-8 p-6 md:p-8">
						{#if activeStep === 1}
							<div in:fade={{ duration: 180 }}>
								<TournamentConfig
									config={{
										format: tsState.format as any,
										courts: tsState.courts,
										scoringMode: tsState.scoringMode,
										pointsPerMatch: tsState.pointsPerMatch,
										maxRepeats: tsState.maxRepeats,
										pairingStrategy: tsState.pairingStrategy,
										courtFormat: tsState.courtFormat,
										customCourtNames: tsState.customCourtNames,
										strictStrategy: tsState.strictStrategy ?? false,
										allowCourtChange: tsState.allowCourtChange ?? true
									}}
									{playerCount}
									onChange={handleConfigChange}
								/>
							</div>
						{:else if activeStep === 2}
							<div in:fade={{ duration: 180 }} class="space-y-6">
								<PlayerList
									items={tsState.players}
									onAdd={handleAddPlayer}
									onRemove={handleRemovePlayer}
									onClear={handleClearPlayers}
									onImport={handleImportPlayers}
									onReorder={handleReorderPlayers}
									title={isTeam ? 'Teams' : 'Players'}
									singularTitle={isTeam ? 'Team' : 'Player'}
								/>

								<!-- Preferred partners config for Mexicano -->
								{#if tsState.format === 'mexicano'}
									<div class="border-t border-white/5 pt-6">
										<PreferredPartners
											pairs={tsState.preferredPartners}
											players={tsState.players}
											onAddPair={handleAddPair}
											onRemovePair={handleRemovePair}
										/>
									</div>
								{/if}
							</div>
						{/if}

						<!-- Large Action button at the bottom of the card content -->
						<div class="mt-4 w-full space-y-3">
							<Button
								variant={isReadyToGenerate ? 'shiny' : 'accent'}
								size="lg"
								onclick={handleGenerate}
								disabled={!isReadyToGenerate}
								fullWidth
								class="bg-[#007AFF] py-3.5 text-xs font-black tracking-wider text-[#020607] uppercase shadow-[0_4px_25px_rgba(0,122,255,0.15)] hover:bg-[#007AFF] disabled:cursor-not-allowed disabled:opacity-50"
							>
								Generate Match Schedule
							</Button>

							{#if !isReadyToGenerate}
								<div
									in:fade
									class="flex items-center justify-center gap-2 rounded-xl border border-warning/20 bg-warning/5 px-3 py-2"
								>
									<Icons name="warning" class="h-3.5 w-3.5 shrink-0 text-warning" />
									<p class="text-[11px] font-black tracking-wider text-warning uppercase">
										Need at least {minPlayers}
										{isTeam ? 'teams' : 'players'} (current: {playerCount})
									</p>
								</div>
							{/if}
						</div>
					</div>
				</GlassCard>

				<!-- History log outside setup card -->
				<div class="mx-auto w-full max-w-4xl pt-16">
					<HistorySection />
				</div>
			</div>
		{/if}
	</div>
</div>
